...
<div class="w3-container w3-display-container">
	...
	<div class="modal-content" style="margin-top:50px;">
		...
		<?php
			$result=mysql_query("select * from tbl_employee ORDER BY status ASC");
			while($row=mysql_fetch_array($result))
			{
				...
				<tr>
					...
					<td class="w3-center"><?php if($row[6]=='active'){ echo " <b class='w3-green '> Active </b> ";}else{ echo " <b class='w3-red'> Deactive </b> ";} ?></td>
					<td><center><a href="editEmployee.php?id=<?php echo $row[0]; ?>" class="btn btn-sm btn-success"> <i class="glyphicon glyphicon-edit"></i> </a> | <?php if($row[6]=='active'){ ?> <a id="delete_employee"  href="?deleteID=<?php echo $row[0]; ?>" class="btn btn-sm btn-info"> <i class="glyphicon glyphicon-arrow-down"></i> </a> <?php }if($row[6]=='deactive'){ ?> <a id="delete_employee"  href="?upID=<?php echo $row[0]; ?>" class="btn btn-sm btn-danger"> <i class="glyphicon glyphicon-arrow-up"></i> </a> <?php } ?></center></td>	
				</tr>
				<?php
			}
		?>	
...
<?php
	...
	{
		mysql_query("update tbl_employee set status='deactive' where empID='".$_REQUEST['deleteID']."'");
	...
	{
		mysql_query("update tbl_employee set status='active' where empID='".$_REQUEST['upID']."'");
		//mysql_query("delete from tbl_employee where empID='".$_REQUEST['deleteID']."'");
		echo "<script>window.location='AddEmployee.php'</script>";
	}
?>
