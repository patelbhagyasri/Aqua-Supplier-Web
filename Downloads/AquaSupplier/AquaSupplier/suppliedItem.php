...
<div class="w3-container w3-display-container">
	...
	?>
		...
		<div class="col-sm-2">
				...
				<div class="input-group">
					  <span class="input-group-addon"><i class="glyphicon glyphicon-minus-sign"></i></span>
					<input type="text" class="form-control input-sm" name="txtMan" id="txtMan" placeholder="Rate" tabindex="2" required />
				</div>
		</div>
		<div class="col-sm-2">
				Supply Date
			...
			<div class="col-sm-1">
					<br>
						...
						if(isset($_POST["btnaddtocart"]))
						{
							...
							if($rate>0)
								{
								mysql_query("insert into temp_invoice(invoice_id,product_name,product_rate,uom,product_qty,amount,invoice_date,invoice_type,orderTime) 
								VALUES ('".$_REQUEST['invoiceID']."','".$_POST["txt_product_name"]."','$rate','LTR','$qty','$totalRate',
								'$invoiceDate','Retail Invoice','$timestamp')");	
								}
								
							$resultsum=mysql_query("select sum(amount) from temp_invoice where invoice_id='".$_REQUEST['invoiceID']."'");
							while($row1=mysql_fetch_array($resultsum))
							{
								$total_amount=$row1[0];
								
							}
							mysql_query("update final_invoice set amount='$total_amount',net_amount='$total_amount' where invoice_id='".$_REQUEST['invoiceID']."'");
							mysql_query("update statement set amount='$total_amount',credit='$total_amount' where invoice_id='".$_REQUEST['invoiceID']."' and remark='Credit Bill' and status='Unpaid'");
							mysql_query("update statement set amount='$total_amount',debit='$total_amount' where invoice_id='".$_REQUEST['invoiceID']."' and remark='Cash Bill' and status='Paid'");
							
								 echo "<script>window.location='suppliedItem.php?invoiceID=$_REQUEST[invoiceID]'</script>";
		...
		<div class="w3-responsive">
			<table class="w3-table-all w3-hoverable">
				...
				<?php
					...
					mysql_query('set names utf8');
					$result=mysql_query("select ti.temp_invoice_id,ti.product_name,ti.product_rate,ti.product_qty,ti.amount from temp_invoice ti where ti.invoice_id='".$_REQUEST['invoiceID']."'");
							$count=1;
							while($row=mysql_fetch_array($result))
							{
								?>
								...
								<td><center><a href='?id=<?php echo $row['temp_invoice_id'];?>&invoiceID=<?php echo $_REQUEST['invoiceID']; ?>' onClick="return confirm('Do You Want To Delete ?')"><span style="color:red;" class="glyphicon glyphicon-trash"></span></a></center></td>
								</tr>
								</tbody>
								<?php
								$count++;
		...
		<?php
			...
			{
			mysql_query("delete from temp_invoice where temp_invoice_id='".$_REQUEST['id']."'");
