...
<head>
<meta charset="utf-8">
<title>Daily Truck Report</title>
			...
			if(isset($_POST['btnProduct']))
				{
					...
					while($rowTruck=mysql_fetch_array($resTruck))
					{
						...
						<tr style="">
							...
							<th>
								<?php
										$totalDepositeSum=0;
										$resLTCustomer=mysql_query("select cm.customer_name,cm.house_no,sum(wsm.deposit_amount) from water_supply_master wsm,customer_master cm where cm.customer_id=wsm.customer_id and wsm.deposit_amount>0 and wsm.supply_date='$startDate' and wsm.lt_id='$rowLoadTruck[0]' and cm.billType='CASH' GROUP BY cm.customer_name");
							...
							<th>
								<?php
										$totalDepositeSumM=0;
										$resLTCustomerM=mysql_query("select cm.customer_name,cm.house_no,sum(wsm.deposit_amount) from water_supply_master wsm,customer_master cm where cm.customer_id=wsm.customer_id and wsm.deposit_amount>0 and wsm.supply_date='$startDate' and wsm.lt_id='$rowLoadTruck[0]' and cm.billType='MONTHLY' GROUP BY cm.customer_name");
										while($rowLTCustomerM=mysql_fetch_array($resLTCustomerM))
										{
											$totalDepositeSumM=$totalDepositeSumM+$rowLTCustomerM[2];
											$totalDepositeM=$totalDepositeM+$rowLTCustomerM[2];
											?>
											<b style="float:right;">
											<?php echo $rowLTCustomerM[0]." ".$rowLTCustomerM[1]." :-".$rowLTCustomerM[2];?></b><br>
							...
							<th><b style="">Total: <?php echo $totalDepositeSum+$totalDepositeSumM; ?></b></th>
					...
					?>
						<tr style="background-color:#808080;color:#fff;font-weight:bold;">
							...
							<th>Total Amount: <?php echo $totalDeposite+$totalDepositeM; ?></th>
